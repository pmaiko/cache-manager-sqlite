...
class SqliteCacheAdapter {
    ...
    constructor(name, path, options) {
        ...
        this.__name = name
        this.__default_ttl = options.ttl === undefined ? this.__default_ttl : options.ttl
    ...
    _fetch_all(keys, cb) {
        this.db.serialize(() => {
            ...
            const stmt = util.format(SelectKeyStatementPrefix + postFix, this.__name)
        ...
        return promisified(liftCallback(options, callback), cb => {
            ...
            this._fetch_all(keys, (err, rs) => {
                ...
                if (rows.length < rs.length) {
                    process.nextTick(() => this.__purgeExpired())
                }

                return cb(null, rows.map(row => this.__deserialize(row.val)))
            })
        })
    }
    ...
    mset(...args) {
        ...
        */
        ...
        return promisified(callback, cb => {
            const ttl = options.ttl || this.__default_ttl
            const ts = now()
            const expire = ts + ttl
            const binding = tuples.flatMap(t => [...t, ts, expire])

            const postfix = tuples.map(d => generatePlaceHolders(d.length + 2)).join(', ')
            const stmt = util.format(UpsertManyStatementPrefix + postfix, this.__name)
    ...
    set(key, value, ttl, options, callback) {
        ...
        ttl = liftFirst('number', ttl) || options.ttl || this.__default_ttl

        return promisified(callback, cb => {
            this.db.serialize(() => {
                const stmt = util.format(UpsertStatement, this.__name)
                const ts = now()
                const binding = {$key: key, $val: this.__serialize(value), $created_at: ts, $expire_at: ts + ttl}
    ...
    del(key, options, callback) {
        return promisified(liftCallback(options, callback), cb => {
            this.db.serialize(() => {
                const stmt = util.format(DeleteStatement, this.__name)
    ...
    reset(callback) {
        return promisified(liftCallback(callback), cb => {
            this.db.serialize(() => {
                const stmt = util.format(TruncateStatement, this.__name)
    ...
    __purgeExpired() {
        this.db.serialize(() => {
            const stmt = util.format(PurgeExpiredStatement, this.__name)
            const ts = now()
